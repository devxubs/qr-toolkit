...
interface QRStoreState {
   ...
   setFormValue: <K extends keyof QRFormValues>(
      typeKey: K,
      values: Partial<QRFormValues[K]>,
   ) => void;
   styling: QRStyling;
   ...
   addHistoryItem: (
      item: Omit<QRHistoryItem, "id" | "timestamp" | "isFavorite">,
   ) => void;
...
export const useQRStore = create<QRStoreState>((set, get) => ({
   ...
   addHistoryItem: (item) => {
      ...
      set((state) => {
         ...
         const isDuplicate = state.history
            .slice(0, 5)
            .some((h) => h.rawText === item.rawText);
