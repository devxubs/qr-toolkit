...
export const Header: React.FC = () => {
  ...
  const tabs: { id: NavigationTab; label: string; icon: React.ReactNode; badge?: number }[] = [
    ...
    { id: 'camera-scanner', label: 'Camera Scan', icon: <Camera className="w-4 h-4" /> },
    { id: 'upload-scanner', label: 'Image Scan', icon: <Upload className="w-4 h-4" /> },
    { id: 'batch', label: 'Batch Tools', icon: <Layers className="w-4 h-4" /> },
    { id: 'history', label: 'History', icon: <History className="w-4 h-4" />, badge: history.length },
    { id: 'safety', label: 'Safety Check', icon: <ShieldCheck className="w-4 h-4" /> },
  ];
        ...
        <div className="flex items-center justify-between h-16 gap-4">
          ...
          <div className="flex items-center gap-3 shrink-0">
            ...
            <div>
              ...
              <p className="text-[11px] text-slate-400 hidden sm:block">Generate • Scan • Validate • Security</p>
            </div>
          </div>
